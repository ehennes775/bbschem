...
bb_redo_action_activate(GAction *action, GVariant *parameter)
{
    GtkWidget *window = bb_main_window_get_current_document_window(
        bb_redo_action_get_window(BB_REDO_ACTION(action))
        );

    if (BB_IS_CLIPBOARD_SUBJECT(window))
    {
        bb_clipboard_subject_redo(BB_CLIPBOARD_SUBJECT(window));
    }
}
...
bb_redo_action_dispose(GObject *object)
{
    // BbRedoAction* privat = BbRedo_ACTION_GET_PRIVATE(object);
}
...
bb_redo_action_get_enabled(GAction *action)
{
    ...
    return
        ...
        bb_clipboard_subject_get_can_redo(BB_CLIPBOARD_SUBJECT(window));
}
...
bb_redo_action_get_property(GObject *object, guint property_id, GValue *value, GParamSpec *pspec)
{
    switch (property_id)
    {
        ...
        case PROP_WINDOW:
            g_value_set_object(value, bb_redo_action_get_window(BB_REDO_ACTION(object)));
            break;
...
bb_redo_action_register()
{
    bb_redo_action_get_type();
}
...
bb_redo_action_set_property(GObject *object, guint property_id, const GValue *value, GParamSpec *pspec)
{
    ...
    {
        case PROP_WINDOW:
            bb_redo_action_set_window(BB_REDO_ACTION(object), BB_MAIN_WINDOW(g_value_get_object(value)));
            break;
