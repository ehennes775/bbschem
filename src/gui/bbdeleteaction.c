...
bb_delete_action_activate(GAction *action, GVariant *parameter)
{
    GtkWidget *window = bb_main_window_get_current_document_window(
        bb_delete_action_get_window(BB_DELETE_ACTION(action))
        );

    if (BB_IS_CLIPBOARD_SUBJECT(window))
    {
        bb_clipboard_subject_delete(BB_CLIPBOARD_SUBJECT(window));
    }
}
...
bb_delete_action_dispose(GObject *object)
{
    // BbDeleteAction* privat = BbDelete_ACTION_GET_PRIVATE(object);
}
...
bb_delete_action_get_enabled(GAction *action)
{
    ...
    return
        ...
        bb_clipboard_subject_get_can_delete(BB_CLIPBOARD_SUBJECT(window));
}
...
bb_delete_action_get_property(GObject *object, guint property_id, GValue *value, GParamSpec *pspec)
{
    switch (property_id)
    {
        ...
        case PROP_WINDOW:
            g_value_set_object(value, bb_delete_action_get_window(BB_DELETE_ACTION(object)));
            break;
...
bb_delete_action_register()
{
    bb_delete_action_get_type();
}
...
bb_delete_action_set_property(GObject *object, guint property_id, const GValue *value, GParamSpec *pspec)
{
    ...
    {
        case PROP_WINDOW:
            bb_delete_action_set_window(BB_DELETE_ACTION(object), g_value_get_object(value));
            break;
