...
bb_save_action_activate(GAction *action, GVariant *parameter)
{
    ...
    GObject *receiver = bb_save_action_get_receiver(BB_SAVE_ACTION(action));

    if (BB_IS_SAVE_RECEIVER(receiver))
    {
        GError *error = NULL;

        bb_save_receiver_save(
            BB_SAVE_RECEIVER(receiver),
            &error
            );
...
bb_save_action_dispose(GObject *object)
{
    BbSaveAction *action = BB_SAVE_ACTION(object);

    g_return_if_fail(action != NULL);

    bb_save_action_set_receiver(action, NULL);

    G_OBJECT_GET_CLASS(bb_save_action_parent_class)->dispose(object);
}
...
bb_save_action_get_enabled(GAction *action)
{
    ...
    return
        ...
        bb_save_receiver_get_can_save(BB_SAVE_RECEIVER(receiver));
}
...
bb_save_action_get_property(GObject *object, guint property_id, GValue *value, GParamSpec *pspec)
{
    switch (property_id)
    {
        ...
        case PROP_RECEIVER:
            g_value_set_object(value, bb_save_action_get_receiver(BB_SAVE_ACTION(object)));
            break;
...
GObject*
bb_save_action_get_receiver(BbSaveAction *action)
{
...
bb_save_action_set_property(GObject *object, guint property_id, const GValue *value, GParamSpec *pspec)
{
    ...
    {
        case PROP_RECEIVER:
            bb_save_action_set_receiver(BB_SAVE_ACTION(object), g_value_get_object(value));
            break;
...
void
bb_save_action_set_receiver(BbSaveAction *action, GObject* receiver)
{
    g_return_if_fail(action != NULL);
