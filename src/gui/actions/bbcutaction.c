...
bb_cut_action_activate(GAction *action, GVariant *parameter)
{
    ...
    GObject *receiver = bb_cut_action_get_receiver(BB_CUT_ACTION(action));
...
bb_cut_action_dispose(GObject *object)
{
    BbCutAction *action = BB_CUT_ACTION(object);

    g_return_if_fail(action != NULL);

    bb_cut_action_set_receiver(action, NULL);

    G_OBJECT_GET_CLASS(bb_cut_action_parent_class)->dispose(object);
}
...
bb_cut_action_get_enabled(GAction *action)
{
    ...
    return
        BB_IS_CUT_RECEIVER(receiver) &&
        bb_cut_receiver_can_cut(BB_CUT_RECEIVER(receiver));
}
...
void
bb_cut_action_set_receiver(BbCutAction *action, GObject* receiver)
{
    g_return_if_fail(action != NULL);
