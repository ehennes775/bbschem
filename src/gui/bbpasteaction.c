...
bb_paste_action_activate(GAction *action, GVariant *parameter)
{
    ...
    {
        bb_clipboard_subject_paste(BB_CLIPBOARD_SUBJECT(window));
    }
}
...
bb_paste_action_dispose(GObject *object)
{
    // BbPasteAction* privat = BbPaste_ACTION_GET_PRIVATE(object);
}
...
bb_paste_action_get_enabled(GAction *action)
{
    ...
    return
        ...
        bb_clipboard_subject_get_can_paste(BB_CLIPBOARD_SUBJECT(window));
}
