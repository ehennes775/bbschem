...
{
    opener->reader = BB_GEDA_SCHEMATIC_READER(g_object_new(
        BB_TYPE_GEDA_SCHEMATIC_READER,
        NULL
        ));
}
    ...
    )
{
    ...
    bb_geda_schematic_reader_read_async(
        ...
        schematic,
        cancellable,
        (GAsyncReadyCallback) bb_geda_opener_open_ready,
        task
        );
    ...
    )
{
    ...
    bb_geda_schematic_reader_read_finish(
        bb_geda_opener_get_reader(opener),
        result,
        &error
        );
